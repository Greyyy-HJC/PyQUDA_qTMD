...
# --------------------------
# Helper functions (from utils.tools)
# --------------------------
...
# --------------------------
# IO functions (from utils.io_corr)
# --------------------------
...
def get_sample_log_tag(ama, src, sm):
    ...
    sm_tag = str(sm)
    return ama_tag + "_" + src_tag + "_" + sm_tag
...
# --------------------------
# Boosted smearing (from utils.boosted_smearing_pyquda)
# --------------------------
...
# --------------------------
# Proton TMD measurement (from utils.proton_qTMD_pyquda)
# --------------------------
...
# Global parameters
data_dir = "/home/jinchen/git/lat-software/PyQUDA_qTMD/tests/full_TMD/data"
interpolation = "T5"
...
sample_log_file = data_dir + "/sample_log_qtmd/" + str(conf) + "_" + sm_tag
if latt_info.mpi_rank == 0:
    with open(sample_log_file, "a+"):
        pass

# Measurement loop
for ipos, pos in enumerate(src_production):
    sample_log_tag = get_sample_log_tag(str(conf), pos, sm_tag)
    mpi_print(latt_info, f"START: {sample_log_tag}")

    # Get forward propagator with boosted source
    t0 = time.time()
    srcD = source.propagator(latt_info, "point", pos)
    srcDp = boosted_smearing(srcD, w=parameters["width"], boost=parameters["boost_in"])
    mpi_print(latt_info, f"TIME Pyquda: Generating boosted src {time.time() - t0}s")
